...
#if M1_ABZ
	angle_prev = getAngle_ABZ();  //getVelocity(),make sure velocity=0 after power on
#else
	angle_prev = getAngle();  //getVelocity(),make sure velocity=0 after power on
#endif		
...
#if M1_ABZ
		zero_electric_angle = _normalizeAngle(_electricalAngle(sensor_direction*getAngle_ABZ(), pole_pairs));
#else
		zero_electric_angle = _normalizeAngle(_electricalAngle(sensor_direction*getAngle(), pole_pairs));		
#endif				
